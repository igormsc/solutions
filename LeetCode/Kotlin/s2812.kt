    ...
    fun maximumSafenessFactor(grid: List<List<Int>>): Int {
        ...
        while (q.isNotEmpty()) {
            ...
            (0..3).forEach { d ->
                ...
                if (minOf(x,y) >= 0 && maxOf(x, y) < n && grid[x][y] == 0) q.add(x to y) .also { grid[x][y] = grid[i][j] + 1 }
            }
        }

        val pq = PriorityQueue<Triple<Int, Int, Int>>{ a, b -> if (a.first != b.first) b.first - a.first else if (b.second != a.second) b.second - a.second else b.third - a.third }
        pq.add(Triple(grid[0][0], 0, 0))
        while (pq.peek().let{ it.second < n-1 || it.third < n-1}) {
            val (g, i, j) = pq.poll()
            (0..3).forEach { d ->
                val (x, y) = i + dir[d] to j + dir[d+1]
                if (minOf(x,y) >= 0 && maxOf(x, y) < n && grid[x][y] > 0) pq.add(Triple(minOf(g, grid[x][y]), x, y)) .also { grid[x][y] *= -1 }
            }
        }
